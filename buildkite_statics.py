...
def download_builds():
    MAX_DOWNLOAD_PAGES = 1
    page_number = 1
    while page_number <= MAX_DOWNLOAD_PAGES:
        url = f"https://api.buildkite.com/v2/organizations/{organization}/pipelines/{pipeline}/builds?page={page_number}&per_page=100"
...
def analyze():
    MAX_ANALYZE_PAGES = 1
    page_number = 1
    while page_number <= MAX_ANALYZE_PAGES:
        ...
        for build in builds_for_page:
            if build["branch"] == "master":
                for job in build["jobs"]:
                    ...
                    if name == ":darwin: (OpenJDK 11, Xcode)" and job["state"] == "passed":
